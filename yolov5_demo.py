...
class Detector():
    ...
    def __init__(self, opt):
        ...
        self.init_model()
        self.names = get_labels("det_label.txt")
    ...
    def init_model(self):
        ...
        self.inputs_info = runner.get_model_inputs()

        self.output_dict = {x.name: np.zeros(x.shape).astype(x.numpy_data_type()) for x in outputs_info}

        input_shape = self.inputs_info[0].shape  # 输入节点形状
    ...
    def detect(self, im):
        ...
        input_dict = {
           self.inputs_info[0].name: img.astype(np.float16)
        }

        self.r.execute(input_dict, self.output_dict)   # 执行推理
        pred = self.output_dict['output0']
